...
def plot_allan_deviation_components(fs, components, sensor_type, grade, figs_dir):
    """
    ...
    for key, component_data in components.items():
        ...
        ax.loglog(taus, sigma, "-", color=color, linewidth=2, label=label, alpha=0.8)
    ...
    ax.set_ylabel("Allan Deviation [rad/s] or [m/s²]", fontsize=13, fontweight="bold")
    ax.set_title(
...
def plot_allan_deviation(taus, adev, noise_params, sensor_type, grade, figs_dir):
    ...
    ax.set_title(
        f"Allan Variance: {grade.capitalize()} {sensor_type}",
        fontsize=14,
        fontweight="bold",
    )
...
def main():
    ...
    plot_allan_deviation(
        taus_a, adev_a, noise_char["accel"], "Accelerometer", grade, figs_dir
    )
    ...
    print(f"\nGyroscope ({grade}):")
    print(
        f"  Angle Random Walk (ARW):     {np.rad2deg(noise_char['gyro']['angle_random_walk'])*60:.4f} deg/sqrt(hr)"
    )
    print(
        f"  Bias Instability (BI):       {np.rad2deg(noise_char['gyro']['bias_instability'])*3600:.2f} deg/hr"
    )
    print(
        f"  Rate Random Walk (RRW):      {np.rad2deg(noise_char['gyro']['rate_random_walk'])*60:.5f} deg/s/sqrt(hr)"
    )

    print(f"\nAccelerometer ({grade}):")
    print(
        f"  Velocity Random Walk (VRW):  {noise_char['accel']['velocity_random_walk']:.5f} m/s/sqrt(s)"
    )
    print(
        f"  Bias Instability:            {noise_char['accel']['bias_instability']:.6f} m/s^2"
    )
    ...
    for label, key_in, value_out in (
        ...
        print(
            f"  {label:<28} {value_in:11.3e} {value_out:11.3e} "
