...
from core.sensors import NavStateQPVP, wheel_odom_update
...
def generate_vehicle_trajectory(shape="square", duration=80.0, dt=0.01):
    ...
    wheel_speed_true = np.zeros(
        (N, 3)
    )  # Speed frame: [0, v_forward, 0] (book convention)
    ...
    if shape == "square":
        ...
        for k in range(N):
            ...
            if k > 0:
                ...
                v_map = wheel_speed_true[k, 1] * np.array(
                    [np.cos(current_heading), np.sin(current_heading), 0]
                )
                current_pos += v_map * dt

            pos_true[k] = current_pos
            vel_true[k, :2] = wheel_speed_true[k, 1] * np.array(
                [np.cos(current_heading), np.sin(current_heading)]
            )

            # Quaternion (yaw only)
            quat_true[k] = np.array(
                [np.cos(current_heading / 2), 0, 0, np.sin(current_heading / 2)]
            )
...
def add_wheel_noise(
    wheel_speed_true, gyro_true, add_slip=False, slip_intervals=None, seed=DEFAULT_SEED
):
...
def plot_results(t, pos_true, pos_odom, pos_odom_slip, figs_dir):
    ...
    ax.plot(
        pos_odom[:, 0], pos_odom[:, 1], "b--", linewidth=2, label="Wheel Odom (no slip)"
    )
    ax.plot(
        pos_odom_slip[:, 0],
        pos_odom_slip[:, 1],
    ...
    ax.set_title(
        "Wheel Odometry Example: Position Error (Bounded Drift)",
        fontsize=14,
        fontweight="bold",
    )
    ax.legend(fontsize=11)
...
def main():
    ...
    t, pos_true, vel_true, quat_true, wheel_true, gyro_true = (
        generate_vehicle_trajectory("square", duration, dt)
    )
    ...
    print("\nGenerating plots...")
    error_odom, error_slip = plot_results(
        t, pos_true, pos_odom, pos_odom_slip, figs_dir
    )
