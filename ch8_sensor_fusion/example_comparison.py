...
def run_both_fusions(
    ...
    lc_results = run_lc_fusion(
        dataset, use_gating=use_gating, gate_confidence=gate_confidence, verbose=verbose
    )
...
def print_comparison_table(metrics: Dict) -> None:
    ...
    print(
        f"{'RMSE 2D (m)':<25} {lc['rmse_2d']:>15.3f} {tc['rmse_2d']:>15.3f} "
        f"{lc['rmse_2d'] - tc['rmse_2d']:>+11.3f}"
    )
    print(
        f"{'RMSE X (m)':<25} {lc['rmse_x']:>15.3f} {tc['rmse_x']:>15.3f} "
        f"{lc['rmse_x'] - tc['rmse_x']:>+11.3f}"
    )
    print(
        f"{'RMSE Y (m)':<25} {lc['rmse_y']:>15.3f} {tc['rmse_y']:>15.3f} "
        f"{lc['rmse_y'] - tc['rmse_y']:>+11.3f}"
    )
    print(
        f"{'Max Error (m)':<25} {lc['max_error']:>15.3f} {tc['max_error']:>15.3f} "
        f"{lc['max_error'] - tc['max_error']:>+11.3f}"
    )
    print(
        f"{'Mean Error (m)':<25} {lc['mean_error']:>15.3f} {tc['mean_error']:>15.3f} "
        f"{lc['mean_error'] - tc['mean_error']:>+11.3f}"
    )
    print(
        f"{'Final Error (m)':<25} {lc['final_error']:>15.3f} {tc['final_error']:>15.3f} "
    ...
    # Update statistics
    print(
        f"{'UWB Updates Accepted':<25} {lc['n_updates']:>15d} {tc['n_updates']:>15d} "
        f"{lc['n_updates'] - tc['n_updates']:>+11d}"
    )
    print(
        f"{'UWB Updates Rejected':<25} {lc['n_rejected']:>15d} {tc['n_rejected']:>15d} "
        f"{lc['n_rejected'] - tc['n_rejected']:>+11d}"
    )
    if "n_failed" in lc:
        print(f"{'LC Solver Failures':<25} {lc['n_failed']:>15d} {'N/A':>15} {'':>12}")
    print(
        f"{'Acceptance Rate (%)':<25} {lc['acceptance_rate']:>15.1f} {tc['acceptance_rate']:>15.1f} "
    ...
    print("\nSummary:")
    print(
        f"  - {better_rmse} has lower RMSE ({abs(lc['rmse_2d'] - tc['rmse_2d']):.3f}m difference)"
    )
    print(
        f"  - {better_accept} has higher acceptance rate "
        f"({abs(lc['acceptance_rate'] - tc['acceptance_rate']):.1f}% difference)"
    )
    print(
        f"  - LC: {lc['n_updates']} updates, TC: {tc['n_updates']} updates "
...
def plot_comparison(
    ...
    if len(lc_results.get("uwb_positions", [])) > 0:
        ax1.scatter(
            lc_results["uwb_positions"][:, 0],
            lc_results["uwb_positions"][:, 1],
            s=10,
            c="cyan",
            alpha=0.2,
            label="UWB Fixes",
    ...
    ax4.axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f'RMSE: {metrics["lc"]["rmse_2d"]:.2f}m',
    )
    ...
    ax5.axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f'RMSE: {metrics["tc"]["rmse_2d"]:.2f}m',
    )
    ...
    for i, (lc_val, tc_val) in enumerate(zip(lc_values, tc_values)):
        ...
        ax9.text(
            i + width / 2, tc_val, f"{tc_val:.1f}", ha="center", va="bottom", fontsize=8
        )

    # Overall title
    fig.suptitle(
        "Loosely Coupled vs Tightly Coupled Fusion Comparison",
        fontsize=16,
        fontweight="bold",
    )
    ...
    report = {
        ...
        "comparison": {
            "rmse_difference": float(
                metrics["lc"]["rmse_2d"] - metrics["tc"]["rmse_2d"]
            ),
            "better_rmse": (
                "LC" if metrics["lc"]["rmse_2d"] < metrics["tc"]["rmse_2d"] else "TC"
            ),
            "update_ratio": (
                float(metrics["lc"]["n_updates"] / metrics["tc"]["n_updates"])
                if metrics["tc"]["n_updates"] > 0
                else 0.0
            ),
        },
    }
...
def main():
    ...
    parser.add_argument(
        "--save", type=str, default=None, help="Path to save comparison figure"
    )
    parser.add_argument(
        "--report", type=str, default=None, help="Path to save comparison report (JSON)"
    )
    ...
    save_path = (
        args.save if args.save else "ch8_sensor_fusion/figs/lc_tc_comparison.svg"
    )
