...
def plot_tuning_comparison(
    ...
    if len(baseline["nis"]) > 0:
        ...
        ax5.semilogy(
            np.maximum(nis_baseline[::step], floor),
            ".",
            color=color_baseline,
            markersize=1.5,
            alpha=0.5,
        ...
        ax5.semilogy(
            np.maximum(nis_gating[::step], floor),
            ".",
            color=color_gating,
            markersize=1.5,
            alpha=0.5,
    ...
    acceptance_rates = [
        100
        * baseline["n_uwb_accepted"]
        / (baseline["n_uwb_accepted"] + baseline["n_uwb_rejected"]),
        100
        * gating["n_uwb_accepted"]
        / (gating["n_uwb_accepted"] + gating["n_uwb_rejected"]),
        100
        * huber["n_uwb_accepted"]
        / (huber["n_uwb_accepted"] + huber["n_uwb_rejected"]),
        100
        * cauchy["n_uwb_accepted"]
        / (cauchy["n_uwb_accepted"] + cauchy["n_uwb_rejected"]),
    ]
    ...
    fig.suptitle(
        "Tuning & Robust Loss Comparison (NLOS Dataset)", fontsize=16, fontweight="bold"
    )
...
def main():
    ...
    print(
        "  3. Huber loss: Soft inflation of R for outliers (w_R = |r|/delta for |r|>delta)"
    )
    ...
    improvement = (
        100
        * (rmse_baseline - min(rmse_gating, rmse_huber, rmse_cauchy))
        / rmse_baseline
    )
    ...
    print(
        f"    - The filter is over-confident. Ungated NIS has median "
        f"{nis_median:.1f}, against {0.45:.2f} for a consistent 1-DOF filter,"
    )
    print(
        f"      and only {frac_inside:.0f}% of samples fall inside the 3.84 "
        f"gate. R is set from the line-of-sight noise, while roughly half"
    )
    print(
        "      this dataset's ranges carry an NLOS bias an order of "
        "magnitude larger."
    )
    print(
    ...
    print(
        "    - Starved of updates the state drifts, drift inflates the next "
        "innovation, and the rejection feeds itself."
    )
    print(
        "    - The lesson is not that gating is inferior. A hard gate is "
        "only as good as the covariance it tests against; the robust"
    )
    print(
        "      losses survive the same mis-specified R because they scale "
    ...
    plot_tuning_comparison(
        dataset, baseline, gating, huber, cauchy, save_path=save_path
    )
