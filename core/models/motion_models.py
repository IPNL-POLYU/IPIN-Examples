...
class ConstantVelocity1D:
    ...
    def f(x: np.ndarray, u: Optional[np.ndarray] = None, dt: float = 1.0) -> np.ndarray:
        ...
        position, velocity = x
        return np.array([position + velocity * dt, velocity])
...
class ConstantAcceleration2D:
    ...
    def f(x: np.ndarray, u: Optional[np.ndarray] = None, dt: float = 1.0) -> np.ndarray:
        ...
        if x.shape != (6,):
            raise ValueError(
                f"State must be 6D [px,py,vx,vy,ax,ay], got shape {x.shape}"
            )
...
def create_process_noise_continuous_white_acceleration(
    ...
    # 1D block
    Q_1d = q * np.array([[dt**3 / 3, dt**2 / 2], [dt**2 / 2, dt]])
