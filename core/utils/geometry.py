...
def normalize_jacobian_singularities(
    diff: np.ndarray, ranges: np.ndarray, epsilon: float = EPSILON_RANGE
) -> np.ndarray:
    """
...
def check_anchor_geometry(
    ...
    if position is not None:
        ...
        if position.shape != (dim,):
            return (
                False,
                f"Position dimension mismatch: expected {dim}, got {position.shape}",
            )
        ...
        if dim == 2:
            ...
            if np.any(position < min_bounds - margin) or np.any(
                position > max_bounds + margin
            ):
                msg = (
