...
def generate_ch6_strapdown_dataset(
    ...
    t, p_xy, v_xy, yaw = generate_circular_trajectory(
        radius=radius, speed=speed, dt=dt, duration=duration
    )
    ...
    config = {
        ...
        "trajectory": {"type": "circular", "radius_m": radius, "speed_m_s": speed},
        "imu": {
            "rate_hz": 1 / dt,
            "dt_sec": dt,
...
Available presets: """
    ...
    traj_group.add_argument(
        "--speed", type=float, default=1.0, help="Constant speed in m/s (default: 1.0)"
    )
    traj_group.add_argument(
        "--duration",
        type=float,
