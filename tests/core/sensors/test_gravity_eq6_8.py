...
class TestGravityMagnitudeEq6_8(unittest.TestCase):
    ...
    def test_gravity_at_equator(self):
        ...
        self.assertAlmostEqual(
            g, 9.7803, places=6, msg="Gravity at equator should be 9.7803 m/s²"
        )
    ...
    def test_gravity_at_45_degrees(self):
        ...
        self.assertAlmostEqual(
            g, 9.8062, places=3, msg="Gravity at 45° should be approximately 9.806 m/s²"
        )
    ...
    def test_gravity_variation_range(self):
        ...
        # Check bounds
        self.assertGreater(g_min, 9.77, msg="Minimum gravity should be above 9.77 m/s²")
        self.assertLess(g_max, 9.85, msg="Maximum gravity should be below 9.85 m/s²")
    ...
    def test_gravity_at_typical_city_latitudes(self):
        ...
        self.assertAlmostEqual(
            g_tokyo, 9.7976, places=3, msg="Tokyo gravity should be ~9.798 m/s²"
        )

        # New York City, USA: 40.7128° N
        g_nyc = gravity_magnitude_eq6_8(np.deg2rad(40.7128))
        self.assertAlmostEqual(
            g_nyc, 9.8023, places=3, msg="NYC gravity should be ~9.802 m/s²"
        )

        # Singapore: 1.3521° N (near equator)
        g_singapore = gravity_magnitude_eq6_8(np.deg2rad(1.3521))
        self.assertAlmostEqual(
            g_singapore, 9.7804, places=3, msg="Singapore gravity should be ~9.780 m/s²"
        )

        # London, UK: 51.5074° N
        g_london = gravity_magnitude_eq6_8(np.deg2rad(51.5074))
        self.assertAlmostEqual(
            g_london, 9.8117, places=3, msg="London gravity should be ~9.812 m/s²"
        )
...
class TestGravityMagnitudeWithFallback(unittest.TestCase):
    ...
    def test_default_fallback_when_no_latitude(self):
        ...
        self.assertEqual(g, 9.81, msg="Should return default_g when lat_rad is None")
    ...
    def test_default_fallback_with_custom_default(self):
        ...
        self.assertEqual(
            g, custom_g, msg="Should return custom default_g when lat_rad is None"
        )
    ...
    def test_eq6_8_when_latitude_provided(self):
        ...
        self.assertAlmostEqual(
            g, g_expected, places=10, msg="Should use Eq. (6.8) when lat_rad provided"
        )
        self.assertNotEqual(
            g, 9.81, msg="Should NOT return default when latitude provided"
        )
    ...
    def test_default_parameter_values(self):
        ...
        self.assertEqual(g, 9.81, msg="Should default to 9.81 m/s² with no arguments")
...
class TestGravityEdgeCases(unittest.TestCase):
    ...
    def test_very_small_latitude(self):
        ...
        self.assertAlmostEqual(
            g, 9.7803, places=4, msg="Near-zero latitude should give equator gravity"
        )
    ...
    def test_large_latitude_array(self):
        ...
        self.assertEqual(len(gravities), 1000, msg="Should handle array-like inputs")
        self.assertTrue(
            all(9.77 < g < 9.85 for g in gravities),
