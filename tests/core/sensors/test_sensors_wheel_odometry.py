...
class TestWheelSpeedToAttitudeVelocity(unittest.TestCase):
    ...
    def test_lever_arm_zero(self) -> None:
        """Test that zero lever arm gives v^A = v^S (no correction)."""
    ...
    def test_lever_arm_no_rotation(self) -> None:
        """Test that zero rotation gives v^A = v^S (no correction)."""
        v_s = np.array([5.0, 0.0, 0.0])
        omega = np.zeros(3)
    ...
    def test_lever_arm_forward_yaw(self) -> None:
        """Test lever arm correction with forward offset and yaw rotation."""
        v_s = np.array([5.0, 0.0, 0.0])  # 5 m/s forward
        omega = np.array([0.0, 0.0, 1.0])  # 1 rad/s yaw (turning left)
        lever_arm = np.array([0.5, 0.0, 0.0])  # wheel 0.5m forward of IMU

        v_a = wheel_speed_to_attitude_velocity(v_s, omega, lever_arm)

        # Expected: v^A = v^S - [ω×] l
        # [ω×] l = skew([0, 0, 1]) @ [0.5, 0, 0] = [0, 0.5, 0]
        # So v^A = [5, 0, 0] - [0, 0.5, 0] = [5, -0.5, 0]
        expected = np.array([5.0, -0.5, 0.0])
        np.testing.assert_array_almost_equal(v_a, expected)

    def test_lever_arm_lateral_offset(self) -> None:
        """Test lever arm correction with lateral offset."""
        v_s = np.array([3.0, 0.0, 0.0])
        omega = np.array([0.0, 0.0, 0.5])  # yaw rate
        lever_arm = np.array([0.0, 1.0, 0.0])  # wheel 1m to the right

        v_a = wheel_speed_to_attitude_velocity(v_s, omega, lever_arm)

        # [ω×] l = skew([0, 0, 0.5]) @ [0, 1, 0] = [-0.5, 0, 0]
        # v^A = [3, 0, 0] - [-0.5, 0, 0] = [3.5, 0, 0]
        expected = np.array([3.5, 0.0, 0.0])
        np.testing.assert_array_almost_equal(v_a, expected)

    def test_lever_arm_equation_6_11(self) -> None:
        """Test Eq. (6.11): v^A = v^S - [ω×] l explicitly."""
...
class TestAttitudeToMapVelocity(unittest.TestCase):
    ...
    def test_attitude_to_map_90deg_yaw(self) -> None:
        """Test 90° yaw rotation (heading north → heading east)."""
        v_a = np.array([5.0, 0.0, 0.0])  # forward in attitude frame
        # 90° yaw: q = [cos(45°), 0, 0, sin(45°)]
        q = np.array([np.cos(np.pi / 4), 0, 0, np.sin(np.pi / 4)])

        v_m = attitude_to_map_velocity(v_a, q)

        # After 90° yaw, forward becomes right (y direction in map)
...
class TestWheelOdomUpdate(unittest.TestCase):
    ...
    def test_wheel_odom_update_zero_lever_arm(self) -> None:
        """Test complete update with zero lever arm."""
        p0 = np.zeros(3)
    ...
    def test_wheel_odom_update_with_rotation(self) -> None:
        ...
        # v^A = v^S = [4, 0, 0]
        # v^M = C(45°) @ [4, 0, 0] ≈ [2.83, 2.83, 0]
        # p1 = [0.283, 0.283, 0]
        expected_x = 4 * np.cos(np.pi / 4) * 0.1
        expected_y = 4 * np.sin(np.pi / 4) * 0.1
    ...
    def test_wheel_odom_update_multiple_steps(self) -> None:
        """Test wheel odometry integration over multiple steps."""
        p = np.zeros(3)
        q = np.array([1.0, 0.0, 0.0, 0.0])
        v_s = np.array([2.0, 0.0, 0.0])  # constant 2 m/s
        omega = np.zeros(3)
        lever_arm = np.zeros(3)
        dt = 0.01
        n_steps = 100  # 1 second
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_large_lever_arm(self) -> None:
        """Test with large lever arm (stress test)."""
        v_s = np.array([10.0, 0.0, 0.0])
        omega = np.array([0.0, 0.0, 2.0])  # high yaw rate
        lever_arm = np.array([5.0, 0.0, 0.0])  # large offset
    ...
    def test_negative_velocity(self) -> None:
        """Test with negative (backward) velocity."""
        p0 = np.array([10.0, 5.0, 2.0])
        q = np.array([1.0, 0.0, 0.0, 0.0])
        v_s = np.array([-3.0, 0.0, 0.0])  # backing up
        omega = np.zeros(3)
        lever_arm = np.zeros(3)
        dt = 0.1
